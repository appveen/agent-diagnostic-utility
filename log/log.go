...
func (Log *Logger) GetLogger(agentType string, level string, agentName string, agentID string, MaxBackupIndex string, MaxFileSize string, LogRotationType string, logHookURL string, client *http.Client, headers map[string]string) logger.Logger {
	...
	if !(IsKubernetesEnv()) {
		...
		dateFileName := "diagnosticAgent_" + currentTime.Format("2006-01-02") + ".log"
		...
		if strings.Contains(agentType, "AGENT") {
			...
			namer := func() string {
				return "tempdiagnostic.log"
			}
			logShippingFileAppender := appenders.RollingFile(filepath.Join(d, "..", "log", "tempdiagnostic.log"), filepath.Join(d, "..", "log"), "tempdiagnostic.log", true, false, true, 102400, filepath.Join(d, "..", "log", "tempdiagnostic"), namer)
